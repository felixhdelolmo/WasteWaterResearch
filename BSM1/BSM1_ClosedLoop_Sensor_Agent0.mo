within BSM1;
model BSM1_ClosedLoop_Sensor_Agent0
  parameter Real avg_start = 1;
  parameter Real avg_period = 1 / 96;
  parameter Real agent_step = 7 / 24 / 60;
  parameter Real agent_start = 100;
  final Real gamma = exp(log(0.01) / ceil(7 / agent_step));
  ...
  BSM1.Agent agent annotation(Placement(visible = true, transformation(origin = {23.9656,48.6584}, extent = {{-7.85515,-7.85515},{7.85515,7.85515}}, rotation = 0)));
equation
  connect(agent.y,feedback2.u1) annotation(Line(points = {{32.6063,48.6584},{40.8646,48.6584},{40.8646,47.2287},{40.8646,47.2287}}));
  connect(limiter1.y,agent.nh) annotation(Line(points = {{40.1913,68.3274},{46.8938,68.3274},{46.8938,59.6221},{8.373889999999999,59.6221},{8.373889999999999,56.2725},{22.1752,54.9426},{14.5394,54.9426}}));
  connect(operation_cost,agent.oc) annotation(Line(points = {{110.581,79.90300000000001},{-13.8655,79.90300000000001},{-13.8655,52.6796},{22.8828,49.2326},{15.2176,49.2326}}));
  connect(sensor_class_a1.y,agent.o2) annotation(Line(points = {{42,30},{42,38},{6,38},{6,42.7671},{14.5394,42.7671}}, color = {0,0,127}));
...
end BSM1_ClosedLoop_Sensor_Agent0;
