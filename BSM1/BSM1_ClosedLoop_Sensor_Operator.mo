within BSM1;
model BSM1_ClosedLoop_Sensor_Operator
  //parameter Real avg_start = 1;
  ...
  BSM1.Operator plant_operator annotation(Placement(visible = true, transformation(origin = {23.9656,48.6584}, extent = {{-7.85515,-7.85515},{7.85515,7.85515}}, rotation = 0)));
  BSM1.RandomWeatherPulse randomweatherpulse1(startTime = 112) annotation(Placement(visible = true, transformation(origin = {-110.504,77.7311}, extent = {{-10,-10},{10,10}}, rotation = 0)));
equation
  connect(randomweatherpulse1.y,plant_operator.weather) annotation(Line(points = {{-99.504,77.7311},{-89.4366,77.7311},{-89.4366,94.36620000000001},{-7.74648,94.36620000000001},{-7.74648,47.8873},{15.493,47.8873},{15.493,48.5915},{15.493,48.5915}}));
  connect(randomweatherpulse1.y,WWSignal.u) annotation(Line(points = {{-99.5042,77.7311},{-82.35290000000001,77.7311},{-82.35290000000001,76.8908},{-82.35290000000001,76.8908}}));
  connect(plant_operator.y,feedback2.u1) annotation(Line(points = {{32.6063,48.6584},{40.8646,48.6584},{40.8646,47.2287},{40.8646,47.2287}}));
  connect(limiter1.y,plant_operator.nh) annotation(Line(points = {{40.1913,68.3274},{46.8938,68.3274},{46.8938,59.6221},{8.373889999999999,59.6221},{8.373889999999999,56.2725},{22.1752,54.9426},{14.5394,54.9426}}));
  connect(sensor_class_a1.y,plant_operator.o2) annotation(Line(points = {{42,30},{42,38},{6,38},{6,42.7671},{14.5394,42.7671}}, color = {0,0,127}));
  ...
  connect(Temperature.y,tank2.T) annotation(Line(points = {{-95.9199,43.7284},{-20.9964,43.7284},{-21.2184,32.1423},{-21.2184,33.4821}}));
  connect(Temperature.y,tank5.T) annotation(Line(points = {{-95.9199,43.7284},{-90.0356,43.7284},{-90.0356,54.1783},{-10.3203,15.6584},{-10.3203,8.63353},{-9.89818,8.63353}}));
...
equation
  /*when initial() then
    ...
    */
  /*
  when sample(plant_start, avg_period) then
    ...
    operation_cost = gamma1 * (AE + PE + ME) + gamma2 * SP + EF / 10;
 ...
 */
  ...
  EF = ADsensor_Effluent.EF_NH + ADsensor_Effluent.EF_TN;
  operation_cost = gamma1 * (AE + PE + ME) + gamma2 * SP + EF;
  SP = (ADsensor_Waste.TSS + ADsensor_Effluent.TSS) / 1000;
  PE = 0.004 * ADsensor_Recycle.Out.Q + 0.008 * ADsensor_Return.Out.Q + 0.05 * ADsensor_Waste.Out.Q;
  AE = tank3.AE + tank4.AE + tank5.AE;
  ME = tank1.ME + tank2.ME;
...
end BSM1_ClosedLoop_Sensor_Operator;
